...
OMReactive::OMReactive(IOxfActive* context)
    : state(0U),
      active(false),
      busy(false),
      supportDirectDeletion(false),
      supportRestartBehavior(false),
      activeContext(0),
      rootState(0),
      _strand(OXFEventPollerPool::Instance()._ioc.get_executor()) {
...
IOxfReactive::TakeEventStatus OMReactive::handleEvent(IOxfEvent::Ptr ev) {
  ...
  if (isUnderDestruction()) {
    ...
    if ((shouldTerminate() == true) && (shouldDelete() == true)) {
      ...
      // the m_eventGuard is set by the application when there is a guarded
      // triggered operation
      //_mutex.lock();
      ...
      // result with a status which indicates that the item had reached a
      // terminate connector
      if (shouldTerminate()) {
...
void OMReactive::handleEventNotQueued(IOxfEvent::Ptr& ev) {
  ...
  if (ev) {
    ev.reset();  // shouldDeleteAfterConsume is tested inside of the destroy
                 // function
  } else {
...
bool OMReactive::startBehavior(void) {
  ...
  if (isUnderDestruction()) {
    ...
    if ((isBehaviorStarted() == false) || (restartBehaviorEnabled() == true)) {
      setBehaviorStarted();
      // take the default transition
      rootState_entDef();
      // This takes care of transitions without triggering events
      if (shouldCompleteRun()) {
        // generate a dummy event in case the class doesn't receive any external
        // events this causes the runToCompletion() after the default transition
        // to be taken - in the class own thread (for active classes)
        setCompleteStartBehavior(true);

        // IOxfEvent::Ptr event =
        // std::dynamic_pointer_cast<IOxfEvent>(theStartOrTerminationEvent);
...
void OMReactive::handleNotConsumed(
...
IOxfReactive::TakeEventStatus OMReactive::rootState_processEvent(void) {
  ...
  if (isUnderDestruction()) {
    ...
    AnimServices::notifyError(
        "Event ignored - Instance is under destruction\n\n");
#endif
  } else {
    if (rootState == NULL) {
#ifdef _OMINSTRUMENT
...
std::shared_ptr<IOxfTimeout> OMReactive::scheduleTimeout(
    unsigned long delay, const char* targetStateName) {
  // #[ operation scheduleTimeout(unsigned long,Rhp_const_char_pt)
  //  schedule timeout
  IOxfTimeout::Ptr timeout =
      std::dynamic_pointer_cast<IOxfTimeout>(std::make_shared<OMTimeout>(
          std::dynamic_pointer_cast<IOxfReactive>(shared_from_this()),
          OMTimerManager::Instance(), _strand, delay, targetStateName));
  if (timeout != NULL) {
...
IOxfReactive::TakeEventStatus OMReactive::handleEventUnderDestruction(
    IOxfEvent::Ptr& ev) {
  // #[ operation handleEventUnderDestruction(IOxfEvent)
  if ((ev != NULL) && ev->isTypeOf(OMReactiveTerminationEventId)) {
    if (shouldDelete()) {
      if (isDestroyEventResent()) {
        ...
        // When destroy is called the reactive instance is set in under
        // destruction mode
        //	that prevents sending of additional events and the terminate
        // event is sent by the instance to itself.
        // The terminate event must be the last event sent to this instance and
        // since the setting in under destruction mode is done without guarding
        //	(for performance considerations), the terminate event is reset
        // after consumed on the first time to avoid a scenario where, 	due to
        // task scheduling, another event is sent after the destroy event.
        // Sending the terminate event a second time prevents most scheduling
        // races
        //	however it does not prevent all possible scenarios.
        // In case that your scheduler may stop a task in the middle of a
        // regular code (without a blocking call)
        //	for indefinite time (that will let the reactive instance thread
        // dispatch the terminate event twice), 	you should consider
        // using the direct deletion policy.
        (void)send(
            std::dynamic_pointer_cast<IOxfEvent>(theStartOrTerminationEvent));
...
bool OMReactive::sendEvent(const IOxfEvent::Ptr& ev) {
  ...
  if ((isUnderDestruction() == true) &&
    ...
    if ((ev != NULL) && (context != NULL)) {
      ...
      boost::asio::post(_strand, [=]() { handleEvent(ev); });
      result = true;
    }
  }
  return result;
...
int32_t OMReactive::getMaxNullSteps(void) { return maxNullSteps; }

void OMReactive::setMaxNullSteps(int32_t p_maxNullSteps) {
  maxNullSteps = p_maxNullSteps;
}

bool OMReactive::isActive(void) const { return active; }
...
void OMReactive::setGlobalSupportDirectDeletion(
...
void OMReactive::setGlobalSupportRestartBehavior(
    bool p_globalSupportRestartBehavior) {
  globalSupportRestartBehavior = p_globalSupportRestartBehavior;
}

IOxfActive* OMReactive::getActiveContext(void) const { return activeContext; }
...
uint32_t OMReactive::getReactiveInternalState(void) const { return state; }

void OMReactive::setReactiveInternalState(uint32_t p_state) { state = p_state; }

bool OMReactive::isBusy(void) const { return busy; }

void OMReactive::setBusy(bool p_busy) { busy = p_busy; }
...
void OMReactive::setActive(bool p_active) { active = p_active; }
