  ...
  switch (addr->sa_family) {
    ...
    case AF_INET6: {
      if (IN6_IS_ADDR_V4MAPPED(&((struct sockaddr_in6 *)addr)->sin6_addr)) {
        ...
        memcpy(&addr4, 12 + (char *)&(((struct sockaddr_in6 *)addr)->sin6_addr),
               4);
...
int SockUtil::setBroadcast(int fd, bool on) {
  ...
  int ret = setsockopt(fd, SOL_SOCKET, SO_BROADCAST, (char *)&opt,
                       static_cast<socklen_t>(sizeof(opt)));
  if (ret == -1) {
    std::cout << "setsockopt SO_BROADCAST failed" << std::endl;
  }
  return ret;
}
...
int SockUtil::setCloExec(int fd, bool on) {
  ...
  if (flags == -1) {
    std::cout << "fcntl F_GETFD failed" << std::endl;
    return -1;
  }
  if (on) {
    flags |= FD_CLOEXEC;
  ...
  if (ret == -1) {
    std::cout << "fcntl F_SETFD failed" << std::endl;
    return -1;
  }
  return ret;
#else
...
int SockUtil::setNoBlocked(int fd, bool noblock) {
  ...
  if (ret == -1) {
    std::cout << "ioctl FIONBIO failed" << std::endl;
  }
...
class DnsCache {
 public:
  ...
  bool getDomainIP(const char *host, sockaddr_storage &storage,
    ...
    try {
      ...
      if (item) {
        auto addr = getPerferredAddress(item.get(), ai_family, ai_socktype,
                                        ai_protocol);
 ...
 private:
  ...
  std::shared_ptr<struct addrinfo> getSystemDomainIP(const char *host) {
    ...
    if (!answer) {
      std::cout << "getaddrinfo failed: " << host << std::endl;
      return nullptr;
    }
  ...
  struct addrinfo *getPerferredAddress(struct addrinfo *answer, int ai_family,
    ...
    auto ptr = answer;
    while (ptr) {
      if (ptr->ai_family == ai_family && ptr->ai_socktype == ai_socktype &&
...
static int set_ipv6_only(int fd, bool flag) {
  ...
  if (ret == -1) {
    std::cout << "setsockopt IPV6_V6ONLY failed" << std::endl;
  }
  return ret;
}
...
static int bind_sock6(int fd, const char *ifr_ip, uint16_t port) {
  ...
  if (1 != inet_pton(AF_INET6, ifr_ip, &(addr.sin6_addr))) {
    if (strcmp(ifr_ip, "0.0.0.0")) {
      std::cout << "inet_pton to ipv6 address failed: " << ifr_ip << std::endl;
    }
    addr.sin6_addr = IN6ADDR_ANY_INIT;
  }
  if (::bind(fd, (struct sockaddr *)&addr, sizeof(addr)) == -1) {
    std::cout << "Bind socket failed: " << get_uv_errmsg(true) << std::endl;
    return -1;
  }
  return 0;
}
...
static int bind_sock4(int fd, const char *ifr_ip, uint16_t port) {
  ...
  if (1 != inet_pton(AF_INET, ifr_ip, &(addr.sin_addr))) {
    if (strcmp(ifr_ip, "::")) {
      std::cout << "inet_pton to ipv4 address failed: " << ifr_ip << std::endl;
    }
...
int SockUtil::connect(const char *host, uint16_t port, bool async,
  ...
  if (sockfd < 0) {
    std::cout << "Create socket failed: " << host << std::endl;
    return -1;
  }
  ...
  std::cout << "Connect socket to " << host << " " << port
            << " failed: " << get_uv_errmsg(true) << std::endl;
  close(sockfd);
  return -1;
}
...
int SockUtil::listen(const uint16_t port, const char *local_ip, int back_log) {
  ...
  if ((fd = (int)socket(family, SOCK_STREAM, IPPROTO_TCP)) == -1) {
    std::cout << "Create socket failed: " << get_uv_errmsg(true) << std::endl;
    return -1;
  }
  ...
  if (::listen(fd, back_log) == -1) {
    std::cout << "Listen socket failed: " << get_uv_errmsg(true) << std::endl;
    close(fd);
    return -1;
  }
...
void for_each_netAdapter_posix(FUN &&fun) {  // type: struct ifreq *
  ...
  if (-1 == ioctl(sockfd, SIOCGIFCONF, &ifconf)) {  // 获取所有接口信息
    std::cout << "ioctl SIOCGIFCONF failed: " << get_uv_errmsg(true)
              << std::endl;
    close(sockfd);
    return;
  }
...
int SockUtil::dissolveUdpSock(int fd) {
  ...
  if (-1 == ::connect(fd, (struct sockaddr *)&addr, addr_len) &&
    ...
    std::cout << "Connect socket AF_UNSPEC failed: " << get_uv_errmsg(true)
              << std::endl;
    return -1;
  }
  return 0;
...
string SockUtil::get_ifr_mask(const char *if_name) {
  ...
  if ((ioctl(fd, SIOCGIFNETMASK, &ifr_mask)) < 0) {
    std::cout << "ioctl SIOCGIFNETMASK on " << if_name
              << " failed: " << get_uv_errmsg(true) << std::endl;
    close(fd);
    return "";
  }
...
string SockUtil::get_ifr_brdaddr(const char *if_name) {
  ...
  if ((ioctl(fd, SIOCGIFBRDADDR, &ifr_mask)) < 0) {
    std::cout << "ioctl SIOCGIFBRDADDR failed: " << get_uv_errmsg(true)
              << std::endl;
    close(fd);
    return "";
  }
...
static void clearMulticastAllSocketOption(int socket) {
  ...
  // This option is defined in modern versions of Linux to overcome a bug in the
  // Linux kernel's default behavior. When set to 0, it ensures that we receive
  // only packets that were sent to the specified IP multicast address, even if
  // some other process on the same system has joined a different multicast
  // group with the same port number.
  int multicastAll = 0;
...
int SockUtil::setMultiTTL(int fd, uint8_t ttl) {
  ...
  if (ret == -1) {
    std::cout << "setsockopt IP_MULTICAST_TTL failed" << std::endl;
  }
#endif
...
int SockUtil::setMultiIF(int fd, const char *local_ip) {
  ...
  ret =
      setsockopt(fd, IPPROTO_IP, IP_MULTICAST_IF, (char *)&addr, sizeof(addr));
  if (ret == -1) {
    std::cout << "setsockopt IP_MULTICAST_IF failed" << std::endl;
  }
#endif
  clearMulticastAllSocketOption(fd);
  return ret;
...
int SockUtil::joinMultiAddr(int fd, const char *addr, const char *local_ip) {
  ...
  if (ret == -1) {
    std::cout << "setsockopt IP_ADD_MEMBERSHIP failed: " << get_uv_errmsg(true)
              << std::endl;
  }
#endif
  clearMulticastAllSocketOption(fd);
...
int SockUtil::leaveMultiAddr(int fd, const char *addr, const char *local_ip) {
  ...
  ret = setsockopt(fd, IPPROTO_IP, IP_DROP_MEMBERSHIP, (char *)&imr,
                   sizeof(struct ip_mreq));
  if (ret == -1) {
    std::cout << "setsockopt IP_DROP_MEMBERSHIP failed: " << get_uv_errmsg(true)
              << std::endl;
  }
#endif
  clearMulticastAllSocketOption(fd);
...
int SockUtil::joinMultiAddrFilter(int fd, const char *addr, const char *src_ip,
  ...
  if (ret == -1) {
    std::cout << "setsockopt IP_ADD_SOURCE_MEMBERSHIP failed: "
              << get_uv_errmsg(true) << std::endl;
  }
#endif
  clearMulticastAllSocketOption(fd);
  return ret;
...
int SockUtil::leaveMultiAddrFilter(int fd, const char *addr, const char *src_ip,
  ...
  ret = setsockopt(fd, IPPROTO_IP, IP_DROP_SOURCE_MEMBERSHIP, (char *)&imr,
                   sizeof(struct ip_mreq_source));
  if (ret == -1) {
    std::cout << "setsockopt IP_DROP_SOURCE_MEMBERSHIP failed: "
              << get_uv_errmsg(true) << std::endl;
  }
#endif
  clearMulticastAllSocketOption(fd);
