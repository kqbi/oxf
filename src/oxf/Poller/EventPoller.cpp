...
#define toPoller(epoll_event)                        \
  (((epoll_event)&EPOLLIN) ? Event_Read : 0) |       \
...
EventPoller::EventPoller(ThreadPool::Priority priority) {
  ...
  if (_epoll_fd == -1) {
    throw runtime_error(StrPrinter << "Create epoll fd failed: "
                                   << get_uv_errmsg());
  }
  ...
  // 添加内部管道事件
  if (addEvent(_pipe.readFD(), Event_Read,
...
int EventPoller::addEvent(int fd, int event, PollEventCB cb) {
  TimeTicker();
  if (!cb) {
    std::cout << "PollEventCB is empty" << std::endl;
    return -1;
  }
  ...
  if (isCurrentThread()) {
    ...
    if (fd >= FD_SETSIZE || _event_map.size() >= FD_SETSIZE) {
      std::cout << "select() can not watch fd bigger than " << FD_SETSIZE
                << std::endl;
      return -1;
    }
#endif
...
int EventPoller::delEvent(int fd, PollDelCB cb) {
  ...
  if (isCurrentThread()) {
#if defined(HAS_EPOLL)
    bool success = epoll_ctl(_epoll_fd, EPOLL_CTL_DEL, fd, nullptr) == 0 &&
                   _event_map.erase(fd) > 0;
    cb(success);
    return success ? 0 : -1;
#else
...
inline void EventPoller::onPipeEvent() {
  ...
  _list_swap.for_each([&](const Task::Ptr &task) {
    try {
      ...
      std::cout << "Exception occurred when do async task: " << ex.what()
                << std::endl;
    }
  });
}

void EventPoller::wait() { lock_guard<mutex> lck(_mtx_running); }

const thread::id &EventPoller::getThreadId() const { return _loop_thread_id; }
    ...
    while (!_exit_flag) {
      ...
      for (int i = 0; i < ret; ++i) {
        ...
        try {
          ...
          std::cout << "Exception occurred when do event task: " << ex.what()
                    << std::endl;
        }
      }
    }
...
uint64_t EventPoller::flushDelayTask(uint64_t now_time) {
  ...
  for (auto it = task_copy.begin();
    ...
    try {
      ...
      std::cout << "Exception occurred when do delay task: " << ex.what()
                << std::endl;
    }
  }
...
EventPollerPool::EventPollerPool() {
  auto size =
      addPoller("event poller", s_pool_size, ThreadPool::PRIORITY_HIGHEST, true,
                s_enable_cpu_affinity);
  std::cout << "EventPoller created size: " << size << std::endl;
}
