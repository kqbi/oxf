...
class OMReactive : public IOxfReactive {
 public:
 ...
 public:
  ...
  void cancel(IOxfTimeout::Ptr &timeout);
  ...
  virtual bool cancelTimeout(const IOxfTimeout::Ptr & /*timeout*/);
  ...
  // It may be overridden in user reactive class to provide processing of this
  // situation. For example, user can destroy event, if event was created
  // dynamically.
  // ## operation handleEventNotQueued(IOxfEvent)
  virtual void handleEventNotQueued(IOxfEvent::Ptr &ev);
  ...
  // signal that there is a null transition to be taken (called by the generated
  // code)
  ...
  virtual bool send(const IOxfEvent::Ptr &ev);
 ...
 protected:
  ...
  virtual void handleNotConsumed(
      IOxfEvent::Ptr & /*ev*/, IOxfReactive::EventNotConsumedReason /*reason*/);

  // This operation is called when the setting of a timeout failed.
  // This happens when the timer manager cannot add the timeout to the waiting
  // timeouts heap The user should override it in derived classes to handle the
  // error. For example the user may call
  // OMTimerManager::instance()->cleanupCanceledTimeouts() and then retry
  // setting the timeout
  ...
  virtual void handleTrigger(IOxfEvent::Ptr &ev);
  ...
  virtual IOxfReactive::TakeEventStatus processEvent(IOxfEvent::Ptr &ev);

  // Checks if second call to startBehavior() should cause restart - i.e.
  // restart of the statechart
  ...
  virtual std::shared_ptr<IOxfTimeout> scheduleTimeout(
      unsigned long delay, const char *targetStateName = NULL);
  ...
  void setCurrentEvent(IOxfEvent::Ptr &ev);
 ...
 private:
  ...
  IOxfReactive::TakeEventStatus handleEventUnderDestruction(IOxfEvent::Ptr &ev);
  ...
  bool sendEvent(const IOxfEvent::Ptr &ev);
 ...
 public:
  // ## auto_generated
  static int32_t getMaxNullSteps(void);

  // ## auto_generated
  static void setMaxNullSteps(int32_t p_maxNullSteps);
  ...
  // ## auto_generated
  static void setGlobalSupportDirectDeletion(
      bool p_globalSupportDirectDeletion);
  ...
  // ## auto_generated
  static void setGlobalSupportRestartBehavior(
      bool p_globalSupportRestartBehavior);

  // ## auto_generated
  virtual IOxfActive *getActiveContext(void) const;

  // ## auto_generated
  virtual std::shared_ptr<IOxfEvent> getCurrentEvent(void);

 protected:
  // ## auto_generated
  uint32_t getReactiveInternalState(void) const;

  // ## auto_generated
  void setReactiveInternalState(uint32_t p_state);
 ...
 private:
  ...
  static const uint32_t defaultStateMask;  // ## attribute defaultStateMask

  // OMReactive state mask - a null transition was found/taken
  static const uint32_t
      nullTransitionStateMask;  // ## attribute nullTransitionStateMask

  // OMReactive state mask - the max number of waiting null transitions that
  // OMReactive can handle
  static const uint32_t nullTransitionMask;  // ## attribute nullTransitionMask

  // OMReactive state mask - the instance statechart reached a terminate
  // connector
  static const uint32_t
      terminateConnectorReachedStateMask;  // ## attribute
                                           // terminateConnectorReachedStateMask

  // OMReactive state mask - the instance is under destruction
  static const uint32_t
      underDestructionStateMask;  // ## attribute underDestructionStateMask

  // OMReactive state mask - the reactive instance should be deleted when a
  // terminate connector is reached
  static const uint32_t
      deleteOnTerminateStateMask;  // ## attribute deleteOnTerminateStateMask

  // OMReactive state mask - need to complete the startBehavior() by a start
  // behavior event
  static const uint32_t
      shouldCompleteStartBehaviorStateMask;  // ## attribute
                                             // shouldCompleteStartBehaviorStateMask

  // OMReactive state mask - startBehavior() was called
  static const uint32_t
      behaviorStartedStateMask;  // ## attribute behaviorStartedStateMask

  // Maximum number of null transitions allowed in a single take
  // infinite loop checker.
  static int32_t maxNullSteps;  // ## attribute maxNullSteps

  // The reactive internal state (bit field)
  uint32_t state;  // ## attribute state

  // The instance start behavior event.
  // This event is sent from the instance to itself when the default transition
  // is followed by null transitions.
  std::shared_ptr<OMStartBehaviorEvent>
      theStartOrTerminationEvent;  // ## attribute theStartOrTerminationEvent

  // This flag mark an active-reactive class
  bool active;  // ## attribute active

  // This flag indicate that the reactive instance is currently handling an
  // event.
  bool busy;  // ## attribute busy

  // When set to true, the reactive class should support direct deletion.
  // The user can also set the globalSupportDirectDeletion attribute to affect
  // all the reactive classes in the system. The reactive instance will support
  // direct deletion if either of the attributes is set to true.
  bool supportDirectDeletion;  // ## attribute supportDirectDeletion

  // When set to true, all the reactive classes in the system should support
  // direct deletion. The user can also set the supportDirectDeletion attribute
  // to affect a single reactive instance. The reactive instance will support
  // direct deletion if either of the attributes is set to true.
  static bool
      globalSupportDirectDeletion;  // ## attribute globalSupportDirectDeletion

  // OMReactive state mask - indicate that theTerminateEvent was resent.
  // The terminate event is send twice to reduce the probability of a race
  // between event sending on call to destroy(). When destroy is called the
  // reactive instance is set in under destruction mode that prevents sending of
  // additional events and the terminate event is sent by the instance to
  // itself. The terminate event must be the last event sent to this instance
  // and since the setting in under destruction mode is done without guarding
  // (for performance considerations), the terminate event is reset after
  // consumed on the first time to avoid a scenario where, due to task
  // scheduling, another event is sent after the destroy event. Sending the
  // terminate event a second time prevents most scheduling races however it
  // does not prevent all possible scenarios. In case that your scheduler may
  // stop a task in the middle of a regular code (without a blocking call) for
  // indefinite time (that will let the reactive instance thread dispatch the
  // terminate event twice), you should consider using the direct deletion
  // policy.
  static const uint32_t
      destroyEventResentStateMask;  // ## attribute destroyEventResentStateMask

  // When set to true the instance reacts to sequential calls of startBehavior()
  // by reset of the statechart. The user can also set the
  // globalSupportRestartBehavior static attribute to apply this behavior to all
  // the instances of OMReactive.
  bool supportRestartBehavior;  // ## attribute supportRestartBehavior

  // When set to true all the instances of OMReactive reacts to sequential calls
  // of startBehavior() by reset of the statechart. The user can also set the
  // instance supportRestartBehavior attribute to apply this behavior to
  // specific instance.
  static bool globalSupportRestartBehavior;  // ## attribute
                                             // globalSupportRestartBehavior
 ...
 private:
  ...
  std::weak_ptr<IOxfEvent> _currentEvent;  // ## link currentEvent

  std::mutex _mutex;  // ## link eventGuard
